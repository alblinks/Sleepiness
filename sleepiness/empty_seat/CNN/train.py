...
class Emptyfier(nn.Module):
    def __init__(self):
        super(Emptyfier, self).__init__()
        self.conv1 = nn.Conv2d(3, 16, 3, padding=1)  # Input channels = 3 (RGB)
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(16, 32, 3, padding=1)
        # Adjusted for new image size
        self.fc1 = nn.Linear(32 * 29 * 25, 512)  # Image size: 100x116
        self.fc2 = nn.Linear(512, 2)  # Output: 2 classes (empty, occupied)

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))
        x = self.pool(F.relu(self.conv2(x)))
        x = x.view(-1, 32 * 29 * 25)  # Image size: 100x116
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        # Save the model
        torch.save(model, f'sleepiness/empty_seat/CNN/weights/empty_seat_epoch_{epoch+1}.pt')